...
class ESSlave(Script):
    ...
    def install(self, env):
        ...
        for es_path_data in params.slave_path_data.split(","):
        ...
        # Create install log
        File(params.es_slave_install_log,
             mode=0644,
             owner=params.es_user,
             group=params.es_group,
             content=''
        ...
        cmd = format("cd {es_slave_base_dir}; wget {es_download_url} -O elasticsearch.tar.gz -a {es_slave_install_log}")
        Execute(cmd, user=params.es_user)

        # Install Elasticsearch
        cmd = format("cd {es_slave_base_dir}; tar -xf elasticsearch.tar.gz --strip-components=1")
        Execute(cmd, user=params.es_user)
        ...
        cmd = format("cd {es_slave_base_dir}; rm -fr elasticsearch.tar.gz")
        Execute(cmd, user=params.es_user)
    ...
    def configure(self, env):
        ...
        File(format("{es_slave_conf_dir}/elasticsearch.yml"),
    ...
    def stop(self, env):
        ...
        """
            ...
            """
        pid_file = params.es_slave_pid_file
        ...
        try:
            ...
            show_logs(params.es_slave_log_dir, params.es_user)
            raise
    ...
    def start(self, env):
        ...
        # Start Elasticsearch
        cmd = format("{es_slave_base_dir}/bin/elasticsearch -d -p {es_slave_pid_file}")
        Execute(cmd, user=params.es_user)
